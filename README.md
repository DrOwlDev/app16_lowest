...
| Platform | Data |
|---|---|
| **Windows** | Live Polymarket Gamma + CLOB; live weather APIs for charts |
...
- Deploy: Flutter web on every `main` push
- Data refresh workflow: updates `gh-pages` `data/markets.json` about every **5 minutes**
  ([Actions](https://github.com/DrOwlDev/app16_lowest/actions/workflows/refresh-data.yml))
...
- **Auto-refresh every 3 minutes** (silent; preserves search, strategy, expanded row). Manual Refresh does the same. EOD countdown UI ticks every 30s.
...
Routing (Windows live / export preload):
